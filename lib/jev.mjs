 ...
 * **まだ育っていない論点は、手放さない。**
 *
 * 論点を作った直後は、比べる相手が「さっきの1発言」しかないので、
 * 「続きですか」の判定が当てにならない。実データ（ノベルティの話4発言）で、
 * トートバッグ → 単価と数量 → 予算 と話が進むたびに別の論点が立ち、
 * 同じ話が3つに割れた。発言が MIN_POINT に満たないうちは、
 * はっきり別の話（STAY_YOUNG 未満）でない限り、いまの論点に置く。
 */
export const MIN_POINT = 3;
export const STAY_YOUNG = 0.22;
...
export async function route(text, { current = null, others = [], agendas = [], about = '' } = {}) {
  ...
  // 1b. いまの論点がまだ育っていない。はっきり別の話でなければ、ここに置く
  if (current && (current.count || 0) < MIN_POINT
      && scores.cur != null && scores.cur >= STAY_YOUNG) {
    return { kind: 'stay', pointId: current.id, young: true, scores };
  }
  // 2. 前に出た論点へ戻った
  let best = null;
  for (const o of pts) {
