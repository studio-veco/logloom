 ...
 * 辞書の正誤表を当てる。
 *
 * **話の中身を一緒に渡す。**辞書には「話の中身で絞る」語がある。
 * 「Gib」は Jev の聞き間違いだが、帆の話では直してはいけない。
 * 中身を渡さないと、そういう語は一度も当たらない（渡す前は当たっていなかった）。
 */
export function fixByDict(text, context = null) {
  if (!mods?.ok) return String(text || '');
  return mods.r.applyGlossary(String(text || ''), glossary, context);
}
