...
export class Kaigi {
  constructor({ title = '', onChange = () => {} } = {}) {
    ...
    this.blind = 0;
    // **手で直したものを覚えておく。**同じ誤りは会議の間じゅう繰り返し出るので、
    // 1回直したら、あとの発言には自動で当てる（from → to）
  ...
  async ingest(raw, { at, ms, who = '' }) {
    ...
    // **話の中身を渡す。**会議の題と、これまでに出た言葉。
    // 辞書の「話の中身で絞る」語（Gib → Jev など）は、これが無いと当たらない
    let text = kotoba.fixCasing(kotoba.fixByDict(cleaned, this.context()));
    const fixes = [];
    // **直したところだけを見せる。**文まるごとを並べると、どこが変わったのか読めない
    if (text !== cleaned) fixes.push({ by: '辞書', ...diff(cleaned, text) });

    // 手で直したものを当てる。辞書より後、Jev より先。
    // **利用者が決めた直しは、Jev の推測より強い。**
    for (const [from, to] of this.corrections) {
      if (!text.includes(from)) continue;
      text = text.split(from).join(to);
      fixes.push({ by: '手入力', from, to });
    }
  ...
  /**
   * いまの会議が何の話か。辞書の「話の中身で絞る」語を当てるのに渡す。
   * 題と、議題の見出しと、直近の発言をつないだもの。
   */
  context() {
    const parts = [this.title];
    for (const a of this.agendas) { if (a.name) parts.push(a.name); }
    for (const l of this.log.slice(-8)) { if (!l.noise) parts.push(l.text); }
    return parts.join(' / ');
  }
    ...
    try {
      return await jev.route(text, {
        current: current && {
          ...
          // **まだ何発言しか入っていないかを渡す。**若い論点は手放さない
          count: current.utterances.length,
        },
  ...
  newPoint(agenda) {
    const p = { id: nid('p'), agendaId: agenda.id, name: '', named: false, by: null, utterances: [],
      summary: '', summaryBy: null, summaryAt: 0, keypoints: [],
      decisions: [], actions: [], issues: [], at: Date.now(), touchedAt: Date.now() };
  ...
  nameLater(point, force = false) {
    ...
    if (!point.named && point.utterances.length >= need && !this.naming.has(point.id)) {
      ...
      ai.nameBranch(lines, { kind: '論点', parent: agenda?.name || '' }).then((r) => {
        ...
        // 付けている間に文が直されていたら、付け直す
        if (point.stale) { point.named = false; this.nameLater(point, true); }
      });
    }
  ...
  summarizeLater(point, force = false) {
    ...
    if (this.summarizing.has(point.id)) return;   // 飛んでいる最中。戻ってきたら stale を見て作り直す
    this.summarizing.add(point.id);
    point.stale = false;
    const n = point.utterances.length;
    const us = [...point.utterances];
    const lines = us.map((u) => u.text);
    // **要点に分けて取る。**地図の第3層に置くものなので、長い1文ではなく短い文をいくつか。
    // 読むための1段落は、その要点をつないで作る（AI を呼ぶのは1回で済む）
    ai.keypoints(point.name || 'この話題', lines).then((r) => {
      point.keypoints = (r.items || []).map((it, i) => ({
        id: `${point.id}_k${i}`,
        text: it.text,
        // どの発言から採ったか。**地図から書き起こしへ飛ぶのに要る**
        from: (it.from || []).map((x) => us[x]?.id).filter(Boolean),
      })).filter((k) => k.text);
      point.summary = point.keypoints.map((k) => k.text.replace(/[。.]$/, '')).join('。') + (point.keypoints.length ? '。' : '');
      point.summaryBy = r.by;
      point.summaryAt = n;
      this.summarizing.delete(point.id);
      this.emit({ kind: 'summary', id: point.id });
      // **まとめている間に文が直されていたら、必ずもう一度。**
      // ここを見ないと、直している最中に頼んだ作り直しが捨てられ、
      // 書き起こしだけ直って要約が古い文のまま残る（実測で残った）
      if (point.stale) { point.summaryAt = 0; return this.summarizeLater(point, true); }
      // まとめている間に増えていたら、もう一度
  ...
  // ---- 手で直す ------------------------------------------------------------
  //
  // 聞き取りと辞書と Jev を通しても、残る誤りはある。同じ音のまま別の語に
  // なっているもの（「3小間」が「3個まで」）は、出口では直しようがない。
  // そこを人が直せるようにする。ただの書き換えでは足りない。**直した結果を、
  // 要約と印と、これから来る発言まで通す。**通さないと、書き起こしだけ直って
  // マインドマップが古いままになる。
   ...
   * @param id     直す発言
   * @param next   直したあとの文
   * @param learn  直した語を覚えて、これから来る発言にも当てるか
   * @param sweep  同じ誤りが入っている、ほかの発言も直すか
   */
  async editUtterance(id, next, { learn = true, sweep = true } = {}) {
    const u = this.log.find((l) => l.id === id);
    if (!u) throw new Error('その発言が見つかりません');
    const text = String(next || '').trim();
    if (!text) throw new Error('空にはできません。文を入れてください');
    const before = u.text;
    if (text === before) return { changed: false, learned: null, swept: 0 };

    const d = diff(before, text);
    u.text = text;
    u.edited = true;
    u.fixes = [...(u.fixes || []), { by: '手入力', from: d.from, to: d.to }];
    this.emit({ kind: 'edited', id: u.id });

    // 直した語を覚える。**文をまるごと書き直したときは覚えない。**
    // 長い差分を正誤表に入れると、二度と当たらないものが溜まるだけになる
    const learned = (learn && this.learnable(d)) ? d : null;
    if (learned) this.corrections.set(learned.from, learned.to);

    // 同じ誤りが入っているほかの発言も直す
    const touched = new Set([u.pointId]);
    let swept = 0;
    if (learned && sweep) {
      for (const l of this.log) {
        if (l === u || l.noise || !l.text.includes(learned.from)) continue;
        l.text = l.text.split(learned.from).join(learned.to);
        l.fixes = [...(l.fixes || []), { by: '手入力', from: learned.from, to: learned.to }];
        touched.add(l.pointId);
        this.redoMarks(l);
        swept += 1;
      }
      if (swept) this.emit({ kind: 'swept', n: swept });
    }

    // 印と、見出しと、要約をやり直す
    this.redoMarks(u);
    for (const pid of touched) this.redoPoint(pid);
    return { changed: true, learned, swept };
  }

  /** 正誤表に入れてよい直しか。短い語の入れ替えだけを覚える */
  learnable(d) {
    const from = String(d.from || '').trim();
    const to = String(d.to || '').trim();
    if (!from || !to || from === to) return false;
    if (from.length > 12 || to.length > 16) return false;
    // 読点だけ、送りがなだけ、といった直しは覚えても意味がない
    if (/^[、。，．,.\s！？!?「」『』ーー]+$/.test(from)) return false;
    return true;
  }

  /** 文が変わったので、決まったこと・やること・課題の印を付け直す */
  redoMarks(u) {
    const p = this.point(u.pointId);
    if (p) {
      for (const key of ['decisions', 'actions', 'issues']) {
        const i = p[key].indexOf(u.id);
  ...
  /** 文が変わったので、その論点の見出しと要約を作り直す */
  redoPoint(pointId) {
    const p = this.point(pointId);
    if (!p) return;
    // **見出しも作り直す。**見出しは最初の数発言から付けているので、
    // そこを直したのに名前が古いままだと、地図が誤りを指したままになる
    p.named = false;
    // **飛んでいる最中の頼みに「作り直せ」と伝える。**
    // 直した瞬間に要約や見出しを作っている最中だと、ここからの頼みは
    // 「いま作っています」と見なされて捨てられる
    p.stale = true;
    const agenda = this.agendas.find((a) => a.id === p.agendaId);
    if (agenda) agenda.named = false;
    p.summaryAt = 0;
    this.nameLater(p, true);
    this.summarizeLater(p, true);
  }
    ...
    return {
      ...
      agendas: this.agendas.map((a) => ({
        ...
        points: a.points.map((p) => ({
          ...
          // 第3層。要約を2〜4つに開いたもの。第4層の振り分けは、この下にぶら下がる
          keypoints: this.keypointsOf(p),
  ...
  /**
   * 第3層（要点）と、その下にぶら下げる第4層（振り分け）を組む。
   *
   * **振り分けは、いちばん近い要点の下に付ける。**論点の真下に横並びにしていたとき、
   * 決まったこと・やること・課題が論点ごとに散らばって、地図が読めなかった。
   */
  keypointsOf(p) {
    const keys = (p.keypoints || []).map((k) => ({ ...k, decisions: [], actions: [], issues: [] }));
    if (!keys.length) return [];
    const put = (bucket, ids) => {
      for (const id of ids) {
        const u = this.log.find((l) => l.id === id);
        if (!u) continue;
        const item = { id, text: youyaku.gist(u.text), full: u.text, at: u.at };
        // もとにした発言に入っている要点があれば、そこへ。無ければ字の似ている要点へ
        let k = keys.find((x) => (x.from || []).includes(id));
        if (!k) {
          let best = 0;
          for (const x of keys) { const s = samish(x.text, u.text); if (s > best) { best = s; k = x; } }
        }
        (k || keys[0])[bucket].push(item);
      }
    };
    put('decisions', p.decisions);
    put('actions', p.actions);
    put('issues', p.issues);
    return keys;
  }
   ...
   * 配るための議事録。**仕事で使える形にする。**
   *
   * これまでの出し方は、木をそのまま並べたものだった。読む人は、誰がいつまでに
   * 何をするのかを、毎回本文から読み取り直すことになる。仕事の議事録は、
   * **決定事項・やること（担当と期限つきの表）・残った課題を先に置き、
   * 話の経緯は後ろに回す。**開いてすぐ、自分に関わるところが見つかる形にする。
   */
  async business() {
    const d = new Date(this.startedAt);
    const end = new Date(this.log.length ? this.log[this.log.length - 1].at : Date.now());
    const two = (n) => String(n).padStart(2, '0');
    const day = `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日`;
    const span = `${two(d.getHours())}:${two(d.getMinutes())}〜${two(end.getHours())}:${two(end.getMinutes())}`;
    const mins = Math.max(1, Math.round((end - d) / 60000));

    // 誰が居たか。**分かっている範囲だけ書く。**入口（マイク／会議の音）で分けたもの
    const who = [...new Set(this.transcript().map((l) => l.who).filter(Boolean))];

    const dec = [], act = [], iss = [];
    for (const a of this.agendas) {
      for (const p of a.points) {
        const where = `${a.name || '議題'} ／ ${p.name || '論点'}`;
        for (const i of p.decisions) { const t = this.textOf(i); if (t) dec.push({ text: t, where, id: i }); }
        for (const i of p.actions) { const t = this.textOf(i); if (t) act.push({ text: t, where, id: i }); }
        for (const i of p.issues) { const t = this.textOf(i); if (t) iss.push({ text: t, where, id: i }); }
      }
    }
    let rows = act.map((x) => ({ ...x, who: '', due: '' }));
    try { rows = (await ai.actionTable(act)).rows; } catch { /* 抜けなくても、やることは出す */ }

    const out = [];
    out.push(`# ${this.title} 議事録`, '');
    out.push('| | |', '| --- | --- |');
    out.push(`| 日時 | ${day} ${span}（${mins}分） |`);
    out.push(`| 出席 | ${who.length ? who.join('・') : '記録なし'} |`);
    out.push(`| 議題 | ${this.agendas.map((a) => a.name).filter(Boolean).join('／') || '記録なし'} |`);
    out.push(`| 記録 | LOGLOOM（音声からの自動書き起こし） |`, '');

    out.push('## 決定事項', '');
    if (dec.length) for (const x of dec) out.push(`- ${x.text}　<!-- ${x.where} -->`);
    else out.push('この会議で決まったことはありません。');
    out.push('');

    out.push('## やること', '');
    if (rows.length) {
      out.push('| 担当 | 期限 | 内容 |', '| --- | --- | --- |');
      for (const r of rows) out.push(`| ${r.who || '未定'} | ${r.due || '未定'} | ${r.text} |`);
    } else out.push('持ち帰りの作業はありません。');
    out.push('');

    out.push('## 残った課題', '');
    if (iss.length) for (const x of iss) out.push(`- ${x.text}`);
    else out.push('未解決の課題はありません。');
    out.push('');
    ...
    for (const a of this.agendas) {
      ...
      for (const p of a.points) {
        ...
        for (const k of (p.keypoints || [])) out.push(`- ${k.text}`);
        if (!(p.keypoints || []).length && p.summary) out.push(p.summary);
        out.push('');
      }
    }
    out.push('---', '');
    out.push('この議事録は、会議の音声をその場で文字にして作ったものです。',
      '**決定事項・やること・残った課題は、発言から拾ったものです。**',
      '担当と期限は、発言に出ていたものだけを書いています（出ていないところは「未定」）。',
      '言い回しは読みやすさのために整えてあります。逐語の記録は下の書き起こしにあります。', '');
    out.push('## 書き起こし', '');
    for (const l of this.transcript()) {
      const t = new Date(l.at);
      out.push(`${two(t.getHours())}:${two(t.getMinutes())}:${two(t.getSeconds())}　${l.who ? `[${l.who}] ` : ''}${l.text}`);
    }
    return out.join('\n');
  }
